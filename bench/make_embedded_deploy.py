...
print("=== Step 1: Write kernel ===")
kernel_src = base64.b64decode("{kernel_b64}").decode()
with open("/content/tess_scatter_bench.cu", "w") as f:
    f.write(kernel_src)
print(f"Kernel: {{len(kernel_src)}} bytes")

print("\\n=== Step 2: Generate synthetic tesspack ===")
random.seed(42)
N_CAPOS = 1871
entries = []
data_parts = []
...
total = os.path.getsize("/content/test.tesspack")
...
if r.returncode != 0:
    ...
    r2 = subprocess.run(["/content/tess_scatter_bench",
        "/content/test.tesspack", "/dev/null"],
...
with open(r"C:\Users\Administrator.AVENTADOR\AppData\Local\Temp\opencode\deploy_embedded.py", "w") as f:
    f.write(deploy)

print(f"Written deploy_embedded.py: {len(deploy)} bytes")
