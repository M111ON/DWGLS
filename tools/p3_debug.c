...
int main(int argc,char**argv){
    ...
    if(strcmp(argv[1],"unfold")==0){
        ...
        uint32_t nslots=((total+59)/60)*60;fprintf(stderr,"dbg: total=%u fsz=%ld nslots=%u v=%d\n",total,fsz,nslots,v);
        DtSlotRegion reg;
        if(dt_slot_init_twin(&reg,twin,nslots,PART_BYTES)!=0){
        ...
        for(uint32_t l=0;l<nslots/60;l++)
            for(uint32_t p=0;p<60;p++){
                ...
                if(len==0) continue;
                fprintf(stderr,"dbg: addr=%u f=%u len=%u\n",addr,f,len);uint8_t*p2=dt_slot_ptr(&reg,addr);
