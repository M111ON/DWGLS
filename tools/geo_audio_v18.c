...
// ═══════════════════════════════════════════════════════════
// MAZE FIELD SWITCH v18 — TIME AXIS as coordinate (user correction)
//
// "เราไม่มี แกนกลางที่เป็นตัวกำหนดเวลา มันเลยย้อนไปมาได้"
//   Our previous patterns summed frames → commutative (time-symmetric):
//   swapping frame order changes nothing → pattern can 'go backwards'.
//
// "ถึงเราจะพูดคำซ้ำสองคำ ถ้าเวลามันผ่านไปแล้ว ก็เอามารวมกันไม่ได้"
//   Each occurrence is anchored at ITS OWN time position. Same word at
//   different time = different event, cannot merge.
//
// Fix: TOWER 81 = 9 amplitude × 9 TIME SLOT. Every hit must declare WHERE
// in the word's life it happened (tslot = (frame−start)×9/nframes).
// Pattern becomes time-ordered: same word at same internal rhythm → same
// slot profile; scrambled/other words → different slot occupancy.
// ═══════════════════════════════════════════════════════════
typedef struct {
    uint32_t switch_hist[N_FLOORS][N_CHANNELS];
    uint32_t skip_val[N_FLOORS][N_CHANNELS];
    uint32_t skip_tower[N_FLOORS][N_CHANNELS][TOWER]; // [amp*9 + tslot]
    int n_frames;
} MazePattern;
...
void build_maze(const double mel[][N_MELS], int start, int end, MazePattern *p) {
    ...
    for (int f = start; f <= end; f++) {
        ...
        // TIME SLOT: where in the word's life is this frame? (0..8)
        int tslot = (int)((double)(f - start) * 9 / nf);
        if (tslot > 8) tslot = 8;

        int amp = (int)(top_val[0] * 9);
        if (amp > 8) amp = 8;
        int tower = amp * 9 + tslot;                // time-anchored bin
...
int main(int argc, char **argv) {
    ...
    printf("=== MAZE FIELD SWITCH v18%s — hilbert = structure, not walker ===\n",
           batch ? " BATCH" : "");
    ...
    // Time-distance analysis: same word close in time vs far in time.
    // User: "ถึงเราจะพูดคำซ้ำสองคำ ถ้าเวลามันผ่านไปแล้ว ก็เอามารวมกันไม่ได้"
    // → split same-word pairs by |frame distance| between occurrences.
    int td_bins = 5;
    double td_same_sum[5] = {0}; int td_same_cnt[5] = {0};
                ...
                for (int wb = 0; wb < nwords[fb]; wb++) {
                    ...
                    if (is_same) {
                        ...
                        for (int fl = 0; fl < 4; fl++) {
                            double c = maze_confidence(&patterns[fa][wa], &patterns[fb][wb], fl);
                            same_c[fl] += c;
                            pair_sum += c;
                        }
                        pair_sum /= 4.0;
                        // time distance between the two occurrences (frames)
    ...
    printf("\n  Same-word by TIME DISTANCE (user: เวลาผ่านไปแล้ว เอามารวมกันไม่ได้):\n");
    for (int b = 0; b < td_bins; b++) {
        if (td_same_cnt[b])
            printf("    %-7s: %4d pairs, avg conf=%.4f\n",
                   td_names[b], td_same_cnt[b], td_same_sum[b]/td_same_cnt[b]);
    }
