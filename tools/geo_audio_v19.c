// geo_audio_v19.c — CATCHING MESH (ตาข่ายดักจับ) — user design
//
// "สนามเป็น grid ตารางเหมือนตาข่ายดักจับ data เข้ามา 1 ช่อง รับผิดชอบ
//  1:1000 sample ถ้ามีแนวโน้มแพทเทินเป็นคำพูด mark switch
//  {sample# fingerprint} × f(time)"
//
// Field = static grid. Each CELL is responsible for CELL_SAMPLES of audio.
//   - time = cell index (the TIME axis IS the address — no merging across
//     time: same word at different time lives in different cells)
//   - if the cell's pattern shows SPEECH tendency (energy above floor)
//     → mark switch ON with fingerprint (top-K freq bins) × intensity
//   - word = the set of cells its audio occupies (known-text aligned)
//
// Cell width sweep: --cell 500 | 1000 | 2000 (samples per cell)
// 62.5ms @1000 = formant-stable statistics vs 10ms frames = transient noise
// (hypothesis: this is why v17's 10ms-frame maze collapsed at scale).
//
// Compare: fingerprint overlap (shared top-K bins) + intensity profile
// cosine over the cell sequence (aligned by min cell count).
//
// Build: gcc -O2 -Wall -o tools/geo_audio_v19.exe tools/geo_audio_v19.c -lm
...
// ═══════════════════════════════════════════════════════════
// CATCHING MESH: each cell = CELL_SAMPLES of audio (time address).
//   cell fingerprint = top-K freq bins over the cell's frames
//   cell intensity   = mean amplitude over the cell's frames
//   word pattern     = sequence of cells its aligned span occupies
// Time is the ADDRESS (cell index) — no cross-time merging.
// ═══════════════════════════════════════════════════════════
typedef struct {
    uint8_t  fp[MAX_CELLS][TOP_K];      // fingerprint per cell
    double   inten[MAX_CELLS];          // intensity per cell
    int      n_cells;
    int      cell_samples;
} MeshPattern;
...
// Build mesh over one word's aligned frame span [start,end].
// cell width in FRAMES: cell_samples / HOP_SIZE (-ish; we use frames*HOP
// so cell ≈ cell_samples audio samples).
void build_mesh(const double mel[][N_MELS], int start, int end,
    ...
    for (int ci = 0; ci < p->n_cells; ci++) {
        ...
        // average mel over the cell → 64 channels
        double ch[64] = {0};
        for (int f = f0; f <= f1; f++)
            for (int c = 0; c < 64; c++)
        ...
        // SPEECH TENDENCY: cell mean amplitude above floor → mark switch
        double mean = 0;
        for (int c = 0; c < 64; c++) mean += amp[c];
        mean /= 64.0;

        if (mean < 0.05) { p->inten[ci] = 0; continue; }  // silence cell
...
// Fingerprint overlap: shared top-K bins / 8, averaged over aligned cells
// plus intensity cosine over the cell sequence. Aligned by min cell count.
double mesh_confidence(const MeshPattern *a, const MeshPattern *b) {
    ...
    // intensity profile cosine over cell sequence
...
int main(int argc, char **argv) {
    int cell_samples = 1000;   // default: user's 1:1000
    ...
    printf("=== CATCHING MESH v19 — cell = %d samples (%.1f ms) ===\n",
    ...
    // time-distance split (user insight)
