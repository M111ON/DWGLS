...
/* Argmax of the last decoded position (independent functional oracle:
 * model logits, not the state blob under test). */
...
int main(int argc, char **argv) {
    ...
    /* ── link 1: resume base file + teacher-decode d1 vs live ──
     * This is the actual arch claim (live = resume + teacher-decode).
     * A fresh direct full decode is kept only as a functional diagnostic
     * (next-token identity from model logits): split-batch vs fused-batch
     * histories leave different scratch bytes in the opaque blob, so blob
     * memcmp across batch splits is out of scope by construction. */
    struct llama_context *rc1 = mk_ctx(model);
    if (!rc1) { fprintf(stderr, "FAIL: rc1 ctx\n"); return 1; }
    KVColdBase rb1; kvcb_init(&rb1);
    int32_t *v1 = NULL; uint32_t vn1 = 0; int32_t vsb1 = -1;
    int rc1_ok = (kvcb_load(&rb1, p_base) == 0 &&
                  kvcb_unspill(&v1, &vn1, &vsb1, p_d1) == 0 &&
                  vsb1 == n0 &&
                  kvcb_resume(rc1, 0, &rb1) == rb1.size);
    if (rc1_ok) decode_at(rc1, v1, (int32_t)vn1, vsb1);
    size_t c1_sz = 0; uint8_t *C1 = NULL;
    int chain1 = 0;
    if (rc1_ok) {
        C1 = get_state(rc1, &c1_sz);
        chain1 = (c1_sz == l1_sz && memcmp(C1, L1, l1_sz) == 0);
    }
    if (!chain1 && rc1_ok && c1_sz == l1_sz) {
        size_t off = 0;
        while (off < l1_sz && C1[off] == L1[off]) off++;
        printf("CHAIN1 first-diff @%llu/%llu\n",
               (unsigned long long)off, (unsigned long long)l1_sz);
    }
    printf("CHAIN1: %s (resume base+d1 vs live, %llu B)\n",
           chain1 ? "PASS" : "FAIL", (unsigned long long)l1_sz);
    free(L1);

    /* ── diag: fresh direct full decode, next-token functional check ── */
    struct llama_context *ref = mk_ctx(model);
    if (!ref) { fprintf(stderr, "FAIL: ref ctx\n"); return 1; }
    decode_at(ref, toks, n0 + n1, 0);
    llama_token ref_next = argmax_last(ref, n_vocab);
    printf("DIAG1: direct next-token %s (live=%d ref=%d)\n",
           live_next == ref_next ? "MATCH" : "DIFF",
           (int)live_next, (int)ref_next);
    llama_free(ref);
    free(v1); free(C1); kvcb_clear(&rb1); llama_free(rc1);
