"""Compare stride layouts for word discrimination in geometric audio codec."""
...
# Stride coprime to 20736
for s in [37, 79, 127, 163]:
    layouts[f'stride{s}'] = np.arange(N) * s % N
# Interleave even/odd
interleave = np.zeros(N, dtype=int)
idx = 0
for i in range(N):
    if i % 2 == 0: interleave[i] = idx; idx += 1
for i in range(N):
    if i % 2 == 1: interleave[i] = idx; idx += 1
layouts['interleave'] = interleave
# Reverse
layouts['reverse'] = np.arange(N)[::-1]

# Test each layout
for name, layout in layouts.items():
    # Build dictionary for this layout
    dict_means = {}
    for i in range(1, 41):
        ...
        for j, (s, e) in enumerate(words):
            if j < len(text):
                ...
                if word not in dict_means:
                    dict_means[word] = []
                dict_means[word].append(fp)
    
    # Average fingerprints
    dict_avg = {w: np.mean(np.array(fps), axis=0) for w, fps in dict_means.items()}
    
    # Test on s1-s10 with threshold 0.5
    total_correct = 0; total_words = 0; total_rejected = 0
    for i in range(1, 11):
        wav = f'{TTS_DIR}/s{i}.wav'
        if not os.path.exists(wav): continue
        samples, sr = read_wav_mono(wav)
        ...
        for j, (s, e) in enumerate(words):
            ...
            if best_sim < 0.5:
                ...
                continue
            total_words += 1
            if best_word == true_text[j].lower():
                total_correct += 1
    
    acc = 100 * total_correct / total_words if total_words > 0 else 0
    print(f'{name:12s}: {total_correct}/{total_words} = {acc:.1f}% (rejected {total_rejected})')
