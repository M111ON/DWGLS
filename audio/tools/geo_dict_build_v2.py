"""Build geometric word dictionary with forced alignment (known text → word boundaries)."""
...
def detect_words_energy(mels, min_dur=5):
    """Energy-based word detection."""
    energy = np.sum(mels**2, axis=1)
    thresh = np.percentile(energy, 25)
    in_w = False; words = []
    for i, e in enumerate(energy):
        if e > thresh and not in_w:
            start = i; in_w = True
        elif e <= thresh and in_w:
            if i - start >= min_dur:
                words.append((start, i))
            in_w = False
    if in_w and len(energy) - start >= min_dur:
        words.append((start, len(energy)))
    return words
...
def forced_align(mels, n_words, min_dur=3):
    """Force-align: split audio evenly into n_words segments."""
    n_frames = len(mels)
    words = []
...
# Build dictionary with forced alignment
print('Building dictionary with forced alignment...')
dict_fps = {}  # word -> list of (fp, sentence_idx)
...
for i in range(1, 51):
    ...
    # Force-align
    word_segments = forced_align(mels, len(words))
    ...
    for j, (s, e) in enumerate(word_segments):
        if j < len(words):
            ...
            if word not in dict_fps:
                dict_fps[word] = []
            dict_fps[word].append((fp, i))

print(f'Dictionary: {len(dict_fps)} unique words')
for w, fps in sorted(dict_fps.items(), key=lambda x: -len(x[1]))[:15]:
    print(f'  "{w}": {len(fps)} samples')

# Save dictionary (just fingerprints, drop sentence index)
dict_save = {k: np.array([fp for fp, _ in v]) for k, v in dict_fps.items()}
np.save('geo_dict_fps_v2.npy', dict_save)
print(f'\nSaved dictionary to geo_dict_fps_v2.npy')

# Test on held-out sentences (s41-s50)
...
for i in range(41, 51):
    ...
    # Force-align test audio
    word_segments = forced_align(mels, len(true_words))
