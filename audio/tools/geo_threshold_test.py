"""Test geo_decoder with confidence threshold."""
...
# Pre-compute mean fingerprints
dict_means = {}
for word, fps in dict_data.items():
    dict_means[word] = np.mean(fps, axis=0)

# Test with different confidence thresholds
for threshold in [0.0, 0.3, 0.4, 0.5, 0.6, 0.7]:
    ...
    for i in range(1, 11):
        ...
        for j, (s, e) in enumerate(words):
            ...
            for w2, mean_fp in dict_means.items():
                sim = cosine(fp, mean_fp)
                if sim > best_sim:
                    best_sim = sim; best_word = w2
            
            true_w = true_text[j].lower()
        ...
        if i == 1:  # Show s1 detail for first threshold
            if threshold == 0.0:
                pass  # skip, we already know this
    
    acc = 100 * total_correct / total_words if total_words > 0 else 0
    print(f'Threshold {threshold:.1f}: {total_correct}/{total_words} = {acc:.1f}% (rejected {total_rejected})')
